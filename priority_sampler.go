...
func (s *prioritySampler) sample() []sampleItem {
	...
	// Loop until we sort everything.
	// We're going to sort it in-place,
	// probabilistically moving the highest weights to the front of the slice.
	//   Pick a random number
	//   Move backwards through the slice on each iteration,
	//   and see where the random number fits in the continuum.
	//   If we find where it fits,
	//   sort the item to the next slot towards the front of the slice.
	for remaining > 1 {
		rn := uint(rand.Uint32()) % sumRemaining // rn from [0 to sumRemaining)
		prevSum := uint(0)
